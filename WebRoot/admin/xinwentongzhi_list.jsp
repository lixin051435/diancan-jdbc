...
if (lb==null)
{
 ...
 %>
  ...
  <form name="form2" method="post" action="admin/delall.jsp">
...
%>
	...
	%>
        ...
        <td width="77" align="center"><a href="admin/xinwentongzhi_updt.jsp?id=<%=id%>">修改</a> <a href="admin/delgg.jsp?id=<%=id %>&tablename=xinwentongzhi&lb=<%=lb%>" onClick="return confirm('真的要删除？')">删除</a></td>
...
<p align="center">&nbsp;以上数据共<%=zgs%>条&nbsp;&nbsp;<%=page_record %>条/页　<a href="admin/xinwentongzhi_list.jsp?page=1&lb=<%=lb%>">首页</a>　<a href="admin/xinwentongzhi_list.jsp?page=<%= curpage-1%>&lb=<%=lb%>">上一页</a>　<A href="admin/xinwentongzhi_list.jsp?page=<%= curpage+1%>&lb=<%=lb%>">下一页</A>　<a href="admin/xinwentongzhi_list.jsp?page=<%=zys %>&lb=<%=lb%>">尾页</A>　当前第<FONT color=red><%=curpage %></FONT>页/共<FONT color=red><%=zys %></FONT>页 
